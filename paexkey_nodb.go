...
func main() {
	...
	estimatedURLs := 100000000  // Adjust this number based on your expected workload
	falsePositiveRate := 0.01 // Adjust this rate as needed
	bloomFilter := bloom.NewWithEstimates(uint(estimatedURLs), falsePositiveRate)

	// Initialize the diskv store for saving Bloom filter and processed URLs
	fOptions := &diskv.Options{
		BasePath:     "datastore", // Choose a directory for storing data
		CacheSizeMax: 1024 * 1024, // 1 MB cache
	}
	d := diskv.New(fOptions)

	// Load Bloom filter data from disk
	bloomFilterData, err := d.Read("bloom_filter")
	if err == nil {
	...
	go func() {
		...
		for s.Scan() {
			...
			if bloomFilter.Test([]byte(url)) {
				// Skip this URL, it's already processed
				log.Println("[SKIPPED] " + url)
				continue
			}

			// If the URL is not in the Bloom filter, add it
		...
		if err != nil {
			log.Fatal("Error saving Bloom filter data:", err)
		}
		err = d.Write("bloom_filter", bloomFilterData)
		if err != nil {
			log.Fatal("Error saving Bloom filter data:", err)
		}

		// Save the updated processed URLs to disk
		processedURLsData, err := json.Marshal(processedURLs)
		if err != nil {
			log.Fatal("Error saving processed URLs data:", err)
		}
		err = d.Write("processed_urls", processedURLsData)
		if err != nil {
			log.Fatal("Error saving processed URLs data:", err)
		}
